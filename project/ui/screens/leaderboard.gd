...
func _ready():
  var config_json = SilentWolfAuth.get_data()
  SilentWolf.configure(config_json)
  
  #var resul: Dictionary = await SilentWolf.Scores.save_score("Jonny", 610000).sw_save_score_complete
  #print("Score persisted successfully: " + str(resul.score_id))
...
func add_item(player_name: String, score_value: String) -> void:
  ...
  item.get_node("Score").text = str(TimeUtils.convert_ms_to_time(int(score_value)))
