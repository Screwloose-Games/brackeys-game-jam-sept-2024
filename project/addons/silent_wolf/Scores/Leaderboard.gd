...
@onready var get_silent_wolf_info: Node = $GetSilentWolfInfo


func _ready():
  var config_json = get_silent_wolf_info.get_data()
  SilentWolf.configure(config_json)
  
  #var resul: Dictionary = await SilentWolf.Scores.save_score("Jonny", 600000).sw_save_score_complete
